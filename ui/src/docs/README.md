...
### 🐳 Docker-запуск (рекомендований спосіб)

Запускає фронтенд, бекенд, базу даних та pgAdmin одночасно:
...
Для демонстрації та ручного тестування:
...
│   │   ├── SearchView.vue     # Пошук
│   │   ├── TopView.vue        # ТОП адаптацій
│   │   ├── ProfileView.vue    # Особистий профіль
│   │   ├── AdminView.vue      # Адмін-панель
│   │   └── NotFoundView.vue   # 404 сторінка
│   ├── components/            # UI-компоненти
│   │   ├── DifferencesMap.vue # Інтерактивна карта відмінностей
│   │   ├── FilterPanel.vue    # Бічна панель фільтрів
│   │   ├── LoginModal.vue
│   │   ├── RegisterModal.vue
...
│   │   ├── works.ts           # Твори
│   │   ├── reviews.ts         # Відгуки
│   │   ├── ratings.ts         # Зіркові оцінки
│   │   ├── votes.ts           # Голосування
│   │   ├── favorites.ts       # Обране
│   │   ├── profile.ts         # Профіль користувача
│   │   ├── admin.ts           # Адмін CRUD
│   │   └── types.ts           # DTO / інтерфейси
│   ├── state/                 # Pinia stores
│   │   ├── user.ts            # Auth + профіль
│   │   ├── filters.ts         # Фільтри каталогу
│   │   ├── userRatings.ts     # Зіркові оцінки
│   │   ├── wishlist.ts        # Список "Хочу переглянути"
│   │   ├── favorites.ts       # Обране
│   │   ├── votes.ts           # Голоси
│   │   └── notifications.ts   # Toast-повідомлення
│   ├── composables/           # Composables (use-функції)
│   │   ├── useFilter.ts       # Логіка фільтрації
│   │   ├── useImageFallback.ts
│   │   └── usePersistedRef.ts # localStorage-persistence
│   ├── hooks/                 # Deprecated re-export composables
│   │   └── useFilter.ts
