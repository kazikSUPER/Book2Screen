...
public class AdaptationProfile : Profile
{
    ...
    public AdaptationProfile()
    {
        this.CreateMap<Adaptation, AdaptationDto>()
            ...
            .ForMember(dest => dest.BookRating, opt => opt.MapFrom(src => src.Work != null && src.Work.Reviews.Any(r => r.TargetType == "book")
                ? Math.Round(src.Work.Reviews.Where(r => r.TargetType == "book").Average(r => r.Rating), 1)
                : 0))
            .ForMember(dest => dest.FilmRating, opt => opt.MapFrom(src => src.Work != null && src.Work.Reviews.Any(r => r.TargetType == "adaptation")
                ? Math.Round(src.Work.Reviews.Where(r => r.TargetType == "adaptation").Average(r => r.Rating), 1)
                : 0))
        ...
        this.CreateMap<Work, BookScreenItemDto>()
            ...
            .ForMember(dest => dest.BookRating, opt => opt.MapFrom(src => src.Reviews.Any(r => r.TargetType == "book")
                ? Math.Round(src.Reviews.Where(r => r.TargetType == "book").Average(r => r.Rating), 1)
                : 0))
            .ForMember(dest => dest.FilmRating, opt => opt.MapFrom(src => src.Reviews.Any(r => r.TargetType == "adaptation")
                ? Math.Round(src.Reviews.Where(r => r.TargetType == "adaptation").Average(r => r.Rating), 1)
                : 0))
